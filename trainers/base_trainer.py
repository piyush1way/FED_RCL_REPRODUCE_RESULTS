...
class Trainer:
    ...
    def local_update(self, device, task_queue, result_queue, device_type="cuda"):
        if device_type == "cuda":
            torch.cuda.set_device(device)
        initalize_random_seed(self.args)
        ...
        while True:
            ...
            # Move state_dict to the current device
            state_dict = {k: v.to(device) for k, v in task["state_dict"].items()}
            ...
            setup_inputs = {
                "state_dict": task["state_dict"],
                "device": torch.device(f"cuda:{device}"),  # Ensure GPU usage
                "local_dataset": local_dataset,
                "local_lr": task["local_lr"],
                "global_epoch": task["global_epoch"],
                "trainer": self,
            ...
            # Move local_model to CPU before putting it in the result queue
            local_model = {k: v.cpu() for k, v in local_model.items()}
            result_queue.put((local_model, local_loss_dict))
    ...
    def train(self) -> Dict:
        ...
        # Use a Manager to handle shared resources
        manager = mp.Manager()
        ...
        device_type = "cuda"
        task_queues = [manager.Queue() for _ in range(M)]  # Use manager.Queue
        processes = [
            mp.get_context("spawn").Process(
                target=self.local_update,
                args=(i % ngpus_per_node, task_queues[i], result_queue, device_type),
            )
            for i in range(M)
        ]
        ...
        for epoch in range(self.start_round, self.global_rounds):
            ...
            for i, client_idx in enumerate(selected_client_ids):
                task_queue_input = {
                    "state_dict": {k: v.cpu() for k, v in self.model.state_dict().items()},  # Move to CPU
                    "client_idx": client_idx,
                    "local_lr": current_lr,
                    "global_epoch": epoch,
                }
                if self.args.multiprocessing:
                    ...
                    task_queue = mp.Queue()
                    task_queue.put(task_queue_input)
                    self.local_update(self.device, task_queue, result_queue, device_type)
