...
class Server():
    ...
    def sum_scaled_weights(self, local_weights: List[Dict[str, torch.Tensor]], length_ratios: List[float]) -> Dict[str, torch.Tensor]:
        '''Return the weighted average of client model weights.'''
        avg_weights = {}
        for param_key in local_weights[0]:  # Iterate over each model parameter
            param_stack = torch.stack([weights[param_key] for weights in local_weights], dim=0)
            avg_weights[param_key] = torch.mean(param_stack, dim=0)  # Average across clients
        return avg_weights

    def aggregate(self, local_weights, local_deltas, client_ids, model_dict, current_lr):
        '''Aggregate client models using weighted averaging.'''
        avg_weights = self.sum_scaled_weights(local_weights, length_ratios=[1.0 / len(client_ids)] * len(client_ids))
        return avg_weights
